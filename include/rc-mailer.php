...
if (!class_exists("RC_Mailer")) {
	...
	class RC_Mailer {
		...
		public function __construct() {
			...
			RC()->hook->listen_action("rc_email_lister_update", array($this, "update_messages"));
			RC()->hook->listen_action("rc_email_lister_bulk_update", array($this, "update_messages"));
		...
		public function update_messages() {
			...
			if (isset($payload["folder"])) {
				if (RC()->receiver->select_folder($payload["folder"])) {
					if (RC()->receiver->update_flag($payload["uids"], $payload["flag"], $payload["value"])) {
						RC()->response = new RC_Response(true, "Moved successfully.!", 0, 0, array("folder" => $payload["folder"], "uids" => $payload["uids"]));
					} else {
						RC()->response = new RC_Response(false, imap_last_error(), 0, -1, array("folder" => $payload["folder"], "uids" => $payload["uids"]));
					}
				} else {
