	...
	class RC_PhpImapReceiver implements RC_Receiver {
		...
		public function fetch_message($_uid, $_msgno) {			
			...
			if( $this->text_message != "" || $this->html_message != "" ) {
				...
				/* It's safe to inspect plain text message also */
				if( $this->text_message != "" ) {
					$this->text_message = RC()->helper->construct_html_doc( $this->text_message);
					$this->text_message= RC()->helper->html_to_string($this->text_message, true);
				}				
		...
		public function update_flag($_uids, $_flaq, $_set) {
			...
			if ($flaq != '') {
				if ($_set) {
					/* Set flag */
					$res = imap_setflag_full($this->stream, $_uids, $flaq, FT_UID);
				} else {
					/* Clear flag */
					$res = imap_clearflag_full($this->stream, $_uids, $flaq, FT_UID);
				}
			}
			imap_expunge($this->stream);
			$this->disconnect();
