...
var rcLister = function( _rc ) {
	...
	this.updateCahceFlag = function( _folder, _uid, _flag, _val ) {
		if( typeof this.binders[ _folder ] != 'undefined' ) {
			for( var i = 0; i < this.binders[ _folder ].headers.length; i++ ) {
				if( this.binders[ _folder ].headers[i].uid == _uid ) {
	...
	this.updateFlag = function( _folder, _msgno, _uid, _action ) {
		var _flag = "FLAG";
		var _val = ( _action == "unflag" ) ? false : true;
		rcControllerObj.request = rcControllerObj.prepareRequest( "PUT", "email", "lister", "update", { folder: _folder, uids: _uid, flag: _flag, value: _val } );
		rcControllerObj.dock();
	...
	this.handleUpdateFlagResponse = function( _req, _res ) {
		if( _res.status ) {
			...
			this.controller.notify.show( "Updated Successfully.!", "success", false );
			/* Update local cache */
			this.updateCahceFlag( folder, uid, "flagged", value ) 
			/* Update viewer */
			if( this.controller.viewer.binders[ uid ] ) {
				/* Update the flag icon visibility
				 * Also change the Flagged menu action and text */
				if( value ) {
					this.controller.viewer.binders[ uid ].view.headerPart.find( "span.rc-mail-viewer-flag-flagged" ).show();
					this.controller.viewer.binders[ uid ].view.headerPart.find( "a.rc-mail-viewer-action-flag" ).attr( "data-action", "unflag" );
					this.controller.viewer.binders[ uid ].view.headerPart.find( "a.rc-mail-viewer-action-flag" ).html( '<i class="fa fa-flag-o"></i> Unflagged' );
				} else {
					this.controller.viewer.binders[ uid ].view.headerPart.find( "span.rc-mail-viewer-flag-flagged" ).hide();					
					this.controller.viewer.binders[ uid ].view.headerPart.find( "a.rc-mail-viewer-action-flag" ).attr( "data-action", "flag" );
					this.controller.viewer.binders[ uid ].view.headerPart.find( "a.rc-mail-viewer-action-flag" ).html( '<i class="fa fa-flag"></i> Flagged' );
				}				
			}
		} else {
			/* Notify the user about the failure */
			this.controller.notify.show( "Update operation failed.!", "error", false );
		}
	};
	...
	this.handleBulkAction = function( _field ) {
		...
		var action = _field.attr( "data-action" );
