...
var rcViewer = function( _rc ) {
	...
	this.initMailView = function( _uid, _msgno ) {	
		if( ( this.controller.folder.current.toLowerCase().indexOf( "draft" ) == -1 ) && ( this.controller.folder.current.toLowerCase().indexOf( "save" ) == -1 ) ) {
	...
	this.prepareforPrint = function( _uid, _msgno, _folder ) {
		if( this.binders[ _uid ] ) {
			this.printMail({
	...
	this.handleResponse = function( _req, _res ) {
		...
		if( ! _res.status && _res.message == "login" ) {
			document.location.href = "";
		} else if( _req.task == "load" ) {
